    ...
    return (
        <nav className="w-full py-4 px-6 md:px-12 flex justify-between items-center bg-[#CEDEBD] relative z-50">
            ...
            <div className="hidden md:flex items-center gap-4">
                <button className="px-5 py-2 text-brand-dark font-semibold hover:opacity-80 transition-opacity">
                    Log In
                </button>
                <button className="px-5 py-2 bg-brand-dark text-white font-semibold rounded-lg hover:bg-opacity-90 transition-opacity">
                    Sign Up Free
                </button>
            </div>
            ...
            {isOpen && (
                <div className="absolute top-16 left-0 w-full bg-brand-light shadow-lg flex flex-col items-center py-6 gap-6 md:hidden">
                    <a href="#" className="text-brand-dark font-medium" onClick={() => setIsOpen(false)}>How it Works</a>
                    <a href="#" className="text-brand-dark font-medium" onClick={() => setIsOpen(false)}>Pricing</a>
                    <a href="#" className="text-brand-dark font-medium" onClick={() => setIsOpen(false)}>Explore Recipes</a>
                    <a href="#" className="text-brand-dark font-medium" onClick={() => setIsOpen(false)}>Community</a>
                    <hr className="w-1/2 border-brand-dark opacity-20" />
                    <button className="text-brand-dark font-semibold">Log In</button>
                    <button className="px-5 py-2 bg-brand-dark text-white font-semibold rounded-lg">Sign Up Free</button>
                </div>
            )}
        </nav>
    );
