...
export default function RecipeConfigurationChat() {
  ...
  const DetailsWidget = () => {
    ...
    return (
      <div className=" p-4 rounded-xl border border-[#E0E6D8] mt-2 space-y-4 w-full">
        {/* Time Selection */}
        <div>
          ...
          <div className="flex gap-2 flex-wrap">
            {['15m', '30m', '45m', '1h+'].map(t => (
              <button
                ...
                className={`px-3 py-1 text-sm rounded-lg border transition-colors ${localTime === t ? 'bg-[#A2B886] text-white border-[#A2B886]' : 'bg-white border-[#D1DAC6] text-[#6B7F4F]'}`}
              >
                {t}
              </button>
            ))}
  ...
  // --- Render ---
  return (
    <div className="flex flex-col h-screen w-full mx-auto bg-[#F9F4E8] font-sans text-[#2C3E14]">
      
      {/* Header */}
      <div className="p-4 bg-[#F9F4E8] border-b border-[#E8E0D0] shadow-sm sticky top-0 z-10 flex items-center gap-3">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
            ...
            {msg.sender === 'bot' && (
              <div className="w-8 h-8 rounded-full bg-[#E8EDDE] flex items-center justify-center mr-2 mt-1 flex-shrink-0 text-[#7D9C5B]">
                <ChefHat size={16} />
              </div>
            )}
            ...
            <div className={`max-w-[85%] ${
              msg.sender === 'user' 
                ...
                : 'bg-white border border-[#E8E0D0] text-[#4A5D23] rounded-2xl rounded-tl-none shadow-sm'
        ...
        {isTyping && (
           <div className="flex justify-start items-center ml-10">
             <div className="bg-[#E8EDDE] p-3 rounded-2xl rounded-tl-none shadow-sm flex gap-1">
      ...
      {/* Sticky Input Area */}
      <div className="p-4 bg-[#F9F4E8] sticky bottom-0 z-20">
        <div className="flex items-center gap-2 bg-[#FDFBF7] p-1.5 rounded-full border border-[#E0D8C8] shadow-sm focus-within:ring-2 focus-within:ring-[#A2B886] focus-within:border-transparent transition-all">
          <input
            type="text"
            value={inputValue}
            ...
            className="flex-1 bg-transparent px-4 py-3 outline-none text-[#4A5D23] placeholder-[#B0C1A1] text-sm"
          ...
          <button 
            ...
            className={`p-3 rounded-full text-white transition-all transform ${
              inputValue.trim() ? 'bg-[#7D9C5B] hover:bg-[#6A8E4C] scale-100 shadow-md' : 'bg-[#D1DAC6] scale-95'
            }`}
          >
            <Send size={18} />
          </button>
        ...
        <div className="text-center mt-2">
             <span className="text-[10px] text-[#AAB59B] font-medium">AI can make mistakes. Check your ingredients.</span>
        </div>
      </div>
    </div>
  );
